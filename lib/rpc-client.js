...
class RPCClient {
    ...
    addCallerStack(error, callerStack) {
        let callerStackLines = callerStack.split('\n');
        callerStackLines.splice(0, 3);
        error.stack = (error.stack ? error.stack + '\n' : '') + 'RPC Called from:\n' + callerStackLines.join('\n');

        logError('RPC-Client', error.message);
    ...
    processReply(message) {
        ...
        if (def) {
            ...
            if (data.error) {
                def.reject(this.addCallerStack(data.error, def.callerStack));
            } else {
